...
#define NSLogError(frmt, ...)    do { if (LOG_LEVEL >= 1) { NSLog( (frmt), ## __VA_ARGS__ ); } \
} \
    while (0)
#define NSLogWarn(frmt, ...)     do { if (LOG_LEVEL >= 2) { NSLog( (frmt), ## __VA_ARGS__ ); } \
} \
    while (0)
#define NSLogInfo(frmt, ...)     do { if (LOG_LEVEL >= 3) { NSLog( (frmt), ## __VA_ARGS__ ); } \
} \
    while (0)
#define NSLogVerbose(frmt, ...)  do { if (LOG_LEVEL >= 4) { NSLog( (frmt), ## __VA_ARGS__ ); } \
} \
    while (0)
